        ...
        <div className="overflow-x-auto max-w-[86vw] sm:w-full">
            <div className={classNames(` grid  gap-4 mb-2 mt-4`, `grid-cols-${cols}`)}>
                ...
                {headers.map((header, index) => {
                    ...
                    return (
                        <h3 key={index} className={classNames('text-xs py-auto text-gray-400 whitespace-nowrap  px-2', `col-span-${span}`)}>
                            {header}
                        </h3>
                    );
                })}
                ...
                {values.map((row, rowIndex) => (
                    <React.Fragment key={rowIndex} >
                        {row.map((value, colIndex) => {
                            ...
                            return (
                                <div key={`${rowIndex}-${colIndex}`} className={classNames('text-xs px-2 text-dark-navy my-1.5 ', `col-span-${span}`)}>
                                    {value}
                                </div>
                            );
                        })}
