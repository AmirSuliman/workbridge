...
const SideNavigation: React.FC<SideNavigationProps> = ({ items, collapsed = false }) => {
    ...
    useEffect(() => {
        ...
        return () => { eventEmitter.off(EventTypes.Toggle_Navbar, () => { }) };
    }, [])
        ...
        >
            ...
            <ul>
                {items.map((item, index) => (
                    <li
                        key={index}
                        className={` flex items-center p-3 ps-6 py-4 cursor-pointer transition-colors duration-75 group hover:bg-dark-navy  hover:text-white  ${path === item.path ? 'bg-dark-navy text-white' : 'text-dark-navy'
                            }`}
                        onClick={() => {router.push(item.path); toggleSidebar()}}
                    >
                        <span
                        ...
                        {!collapsed && <span className="mx-2 text-xs whitespace-nowrap">{item.label}</span>}
                    </li>
                ))}
            </ul>
        </nav>
