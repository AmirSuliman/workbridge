...
export const Pagination: FC<Props> = ({
        ...
        for (let i = startPage; i <= endPage; i++) {
            pageNumbers.push(
                <button
                    ...
                    className={`px-4 rounded-[6px] text-sm py-2 ${currentPage === i ? 'bg-dark-navy text-white border-0' : 'bg-white'
    ...
    return (
        <div className={`${styles?.container} flex  justify-center bg-white rounded-md text-sm`}>
            <button
                onClick={() => handlePageChange(currentPage - 1)}
                disabled={currentPage === 1}
                className={`px-3 py-2 border border-light-gray rounded-[6px] ${currentPage === 1
            ...
            <button
                ...
                className={`px-3 py-2 border border-light-gray rounded-[6px]  ${currentPage === totalPages
                    ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                    : 'bg-white'
                    } focus:outline-none`}
            >
